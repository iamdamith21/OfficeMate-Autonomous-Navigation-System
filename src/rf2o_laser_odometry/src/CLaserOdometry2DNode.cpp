...
void CLaserOdometry2DNode::publish()
{
  ...
  if (publish_tf)
  {
    ...
    odom_trans.header.stamp = this->now();  // use publish time so TF is never stale
...
// Re-broadcasts the last known pose with a fresh timestamp at 50 Hz so
// RViz never sees a gap between the scan timestamp and the latest TF.
void CLaserOdometry2DNode::republishTF()
{
  if (!rf2o_ref.is_initialized() || !publish_tf) return;
  geometry_msgs::msg::TransformStamped t;
  t.header.stamp     = this->now();
  t.header.frame_id  = odom_frame_id;
  t.child_frame_id   = base_frame_id;
  t.transform.translation.x = rf2o_ref.robot_pose_.translation()(0);
  t.transform.translation.y = rf2o_ref.robot_pose_.translation()(1);
  t.transform.translation.z = 0.0;
  t.transform.rotation = tf2::toMsg(tf2::Quaternion(
      tf2::Vector3(0.0, 0.0, 1.0),
      rf2o_ref.robot_pose_.rotation().eulerAngles(0, 1, 2)(2)));
...
int main(int argc, char** argv)
{
  ...
  // Scan processing at user-configured freq (default 7 Hz).
  // TF keep-alive at 50 Hz so RViz never extrapolates into the future.
  rclcpp::Rate scan_rate(myLaserOdomNode->freq);
  rclcpp::Rate tf_rate(50.0);
  int tf_ticks = 0;
  const int tf_per_scan = static_cast<int>(50.0 / myLaserOdomNode->freq + 0.5);

  while (rclcpp::ok()) {
    rclcpp::spin_some(myLaserOdomNode);
    myLaserOdomNode->republishTF();
    tf_rate.sleep();
    if (++tf_ticks >= tf_per_scan) {
      tf_ticks = 0;

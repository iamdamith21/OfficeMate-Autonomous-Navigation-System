...
# The Mega is a CH9102 clone (1a86:55d8); officemate.rules only matches a
# genuine 2341:0042 board, so /dev/arduino is never created. by-id is stable
# across replug, unlike ttyACM0.
ARDUINO_DEV="${ARDUINO_DEV:-/dev/serial/by-id/usb-1a86_USB_Single_Serial_5750021289-if00}"
...
case "${1:-status}" in
  start)
    ...
    setsid ros2 launch robot_navigation navigation.launch.py "map:=$MAP" "arduino_dev:=$ARDUINO_DEV" \
